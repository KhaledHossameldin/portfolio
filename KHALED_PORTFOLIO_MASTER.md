...
| 2026-06-26 | **Slack notify — empty-webhook guard + discovered the secret is NOT set (owner action needed).** First post-merge deploy (run 28269050481) was green and the `Notify Slack — success` step RAN, but `$SLACK_WEBHOOK_URL` resolved EMPTY → `curl: (3) URL rejected: Malformed input to a URL function`; `continue-on-error:true` masked it green so nothing posted (a misleading `##[error] exit code 3` in the log). Root cause: **the repo has NO secrets set** — `gh secret list` is empty; the pipeline auth uses repo *Variables* (`AWS_DEPLOY_ROLE_ARN`, `S3_BUCKET`, `CF_DIST_ID`, `CONTACT_API_URL`), and `SLACK_WEBHOOK_URL` was never added as a Secret. Fix (code): added a guard at the top of BOTH notify steps — `if [ -z "$SLACK_WEBHOOK_URL" ]; then echo "…not set — skipping"; exit 0; fi` — so the steps skip cleanly with a clear log line instead of a masked curl error, and post normally once the secret exists. **BLOCKER / owner action (§11):** I cannot set GitHub secrets (and don't hold the webhook value) — the owner must add the repo secret **`SLACK_WEBHOOK_URL`** (the #deployments Incoming Webhook URL) under Settings → Secrets and variables → Actions → Secrets. After that, the next deploy posts the ✅ message (and ❌ on failure) with no further code change. Failure path reasoned: `if: failure()` fires on any prior step error → ❌ message + run link; with the guard it also no-ops cleanly while the secret is absent. | CC | CONFIRMED |
| 2026-06-28 | **Perf — trim Hanken Grotesk to 3 weights (drop unused 700).** Separate `perf/trim-hanken-700` branch off `main` (kept out of the Arabic PR #27 since it's en/de-wide, not RTL-scoped). `--weight-bold: 700` is defined in `globals.css` but has **0 `var()` consumers**, and no inline `fontWeight: 700/bold` exists anywhere in `src/`, so the 700 woff2 set was dead payload on every page. Dropped `"700"` from the Hanken `weight` array in `layout.tsx` (now `["400","500","600"]`). Verified against the production export: built `@font-face` set for Hanken is now `['400','500','600']` with **no 700 face** (parsed the emitted chunk CSS); `tsc --noEmit` clean, `next build` green (16/16 static pages). Mirrors the earlier IBM Plex Sans Arabic 5→4 trim — same dead-weight rationale, applied to the Latin body font. Left the dead `--weight-bold` token in place (harmless, documents intent; re-adding bold later just re-lists the weight). | CC | CONFIRMED |

| 2026-06-26 | **Arabic locale — Stage A scaffolding (of a 5-stage RTL rollout; A→E plan in the plan file).** Added `ar` as a third statically-generated locale that renders RTL with the Arabic font, behind ENGLISH PLACEHOLDER copy (so the build stays green before any Arabic content). Changes: `i18n/routing.ts` locales `+"ar"` (+ `rtlLocales`/`isRtl` helpers); `messages/ar.json` = copy of `en.json` (placeholder, all 11 namespaces); `data/projects.ts` — `Loc<T>` changed `Record<Locale,T>` → `{en;de;ar?}` (ar OPTIONAL during rollout) + new `pick(value,locale)` helper that falls back to English, and the 12 read sites in `SelectedWork`/`ProjectDetail`/`work/[slug]/page.tsx` switched from `x[locale]` → `pick(x,locale)` (chose en-fallback over duplicating 72 placeholder strings — no drift, smaller diff, same outcome); root `layout.tsx` adds `IBM_Plex_Sans_Arabic` via next/font (`subsets:["arabic"]`, weights 300–700, `preload:false`) and the pre-paint `themeInit` script now also sets `documentElement.lang`+`dir` from the URL's locale segment (`/ar`→rtl) — no FOUC; `[locale]/layout.tsx` drops the old setLang script for a server-rendered `<div dir … style="display:contents">` wrapper (no-JS/crawler direction, zero box); `globals.css` adds `[dir="rtl"]{ --font-serif/-sans/-mono → var(--font-arabic) }`; SEO gains `ar` in all hreflang `languages` maps (root + `[locale]` + `[slug]`) and `socialMeta` OG_LOCALE `ar:"ar_SA"`. Verified: lint + build + tsc clean, **22 static pages** (was 16 — adds `/ar` + `/ar/work/×5`), en/de UNCHANGED (still LTR, 0 `dir=rtl`); compiled CSS chunk ships the `[dir=rtl]` font override + `@font-face` IBM Plex Sans Arabic; live preview `/ar` → `html dir=rtl lang=ar`, body direction rtl, h1 computed font `IBM Plex Sans Arabic`, layout fully mirrored, AR toggle auto-present. **Still TODO (later stages, NOT in A):** B = RTL CSS logical-property fixes + icon mirroring + bidi isolation for embedded Latin/numerals (visible now: the `Khaled.` dot + `—`/`.` punctuation render on the wrong side, arrows not mirrored); C = owner-provided Arabic content (`ar.json` + 12 projects' `ar` fields) — then flip `ar` to required + extend the guard to check `.ar`; D = SA geo-default routing at the root dispatcher; E = full verification (Lighthouse on /ar, RTL visual both themes, motion). NOT deployed — PR opened for review; recommend holding the merge until at least Stage B so an unpolished RTL `/ar` isn't public (it's reachable only via the toggle; root does not auto-route to /ar yet). | CC | CONFIRMED |

| 2026-06-26 | **Arabic locale — Stage B (RTL CSS: logical properties + icon mirroring + bidi isolation).** On the same branch as Stage A (PR #27, still held). globals.css adds three RTL helpers: `[dir="rtl"] .kp-mirror{transform:scaleX(-1)}` (mirror directional icons), `[dir="rtl"] .kp-em{font-style:normal !important}` (Arabic has no italic — `!important` to beat the inline `font-style:italic`), and `.kp-ltr{unicode-bidi:isolate;direction:ltr}` (isolate embedded Latin/numeric runs; no-op in LTR). Physical→logical: `Nav` `marginLeft`→`marginInlineStart` (nav links + the `auto` that pushes the toggle cluster), `Button` icon nudges `marginLeft/Right`→`marginInlineStart/End`, `ProjectDetail` highlight rule `borderLeft+paddingLeft`→`borderInlineStart+paddingInlineStart`. Icon mirroring (`className="kp-mirror"`, icons forward props→svg): ProjectDetail back `ArrowLeft` + see-all `ArrowRight` + link `ArrowUpRight`; SelectedWork card/store/also `ArrowUpRight`×3; Hero + Contact CTA `ArrowUpRight`; Footer `↗` glyph (+display:inline-block). Bidi isolation (`kp-ltr`): the `Khaled.` wordmark (dot now stays trailing/right), Nav mono indices 01–04, SelectedWork card index `NN / NN`, Contact email/GitHub/LinkedIn links. Italics tagged `kp-em`: Hero line2, Stack note, SelectedWork note, Footer signoff. Verified live on `/ar`: wordmark bidi `isolate/ltr` (dot right), `.kp-mirror` computed `scaleX(-1)` (23 icons), `.kp-em` computed `font-style:normal`, CTA arrow mirrored to up-left; en/de unaffected (rules are `[dir=rtl]`-scoped). lint + build + tsc clean, 22 pages. Remaining: a placeholder-only bidi quirk (English sentence-final punctuation in the RTL lead) vanishes once Arabic content lands (Stage C). | CC | CONFIRMED |

| 2026-06-26 | **Arabic locale — Stage D (SA geo-default at the root dispatcher).** Extended `app/src/app/page.tsx`'s client dispatcher (still 100% client JS, static-export-safe, no middleware): **language-first** (instant, no network) — Arabic browser language (`ar*`) → `/ar/`, German (`de*`) → `/de/`; **else** a keyless ipapi country probe (`https://ipapi.co/country/`, AbortController + 1.5s timeout) → **`SA` → `/ar/`**, everyone else → `/en/`; any failure/timeout/abort falls back to `/en/` and a `done` guard prevents a double redirect / hang. `<noscript>` stays `/en/`. EG is unaffected by routing (still `/en`; the per-page `useVisitorCountry` "Based" swap is separate). Verified in `out/index.html`: the built dispatcher contains the `ar`/`de` language branches, the `==='SA'?'/ar/'` country branch, and the ipapi fetch. (Geo paths aren't interactively spoofable in the harness — logic verified in the static output; failure path reasoned: timeout/blocked/non-SA → `/en/`.) lint + build + tsc clean, 22 pages. On branch `feat/app-ar-locale-scaffold` (PR #27). Remaining: C (owner Arabic content) + E (Lighthouse/RTL/motion verification). | CC | CONFIRMED |

| 2026-06-26 | **Arabic locale — Stage C (Arabic content wired) + part of E (RTL/visual verify).** Owner asked CC to translate rather than supply copy, so CC produced native MSA Arabic for the full surface (owner to do a final voice pass). `messages/ar.json` — all 11 namespaces translated (tech proper nouns kept Latin: Flutter, Next.js, AWS, Terraform, BLoC, DevOps, CI/CD…; UI/marketing → MSA; neutral fields kept: urls, hrefs, year, DOI, emailPh, the `This site` panel value). `data/projects.ts` — **regenerated** with `ar` added to every localized field of all 11 projects (title/tagline/role/stack/summary/highlights), via Node `--experimental-strip-types` extraction of the existing en+de → translate → regenerate (one consistent format, no hand-surgery). `Loc<T>` flipped back to `Record<Locale,T>` (ar now REQUIRED — tsc enforces all three); integrity guard extended to validate title/tagline/role/stack for all three locales + summary/highlights for detail projects in all three. Translation choices: brand app-names kept Latin (Sedra Life, QMS, Albruaz, Royake, Orood, QRattel, Drs Space) pending owner's official Arabic; descriptive titles translated ("This portfolio"→"هذا المعرض", "Al Frayan App"→"تطبيق آل فريان", "Sabq App"→"تطبيق سبق"); translatable stack skill-phrases → Arabic ("Production Support"→"دعم الإنتاج", "State Management"→"إدارة الحالة", etc.), tool names stay Latin. Verified: lint + build + tsc clean (22 pages); `out/ar` renders Arabic (home + detail), `out/en` has 0 Arabic leak; live preview `/ar` + `/ar/work/sedra-life` both themes — Arabic in IBM Plex Sans Arabic, RTL correct, embedded Latin tech bidi-isolated inline, arrows mirrored, highlight rule on the right (borderInlineStart), wordmark dot trailing. **Remaining (E):** owner Lighthouse PSI on `/ar` home + a detail route (≥90; CC can't run PSI in-harness) + owner native-Arabic voice review; then this PR (#27, stages A–D + C) can merge → geo sends SA visitors to `/ar`. | CC | CONFIRMED |

| 2026-06-28 | **Arabic locale — Stage E (Lighthouse review) + font-weight trim.** Owner ran Lighthouse on `/ar` via the **dev server** (`localhost:3000`): Desktop **94 / 100 / 96 / 100** (all ≥90 ✓); Mobile **73 / 100 / 96 / 100** — mobile Perf below 90 with LCP 9.1s. Diagnosis: the mobile number is a **dev-server artifact** (Turbopack serves unminified + on-demand compile + no prod caching; the live `/en` `/de` score 99–100 mobile in production). Desktop clearing 94 even in dev corroborates. Real fix path = measure the production static export, not dev. Did one genuine payload win surfaced by the review: **trimmed the IBM Plex Sans Arabic font from 5 weights to 4** (dropped 700 — `--weight-bold` is defined but unused by every `--type-*` token and unreferenced inline). Built the prod export and served `app/out/` statically (`python3 -m http.server 4123`) so the owner can re-run Lighthouse against a prod-representative build. **Optional follow-up (not done, en/de-scoped):** Hanken Grotesk also declares an unused 700 — trimming it is a payload win for all locales but belongs in its own perf commit, not the ar PR. A11y/BP/SEO already 100/96/100 on `/ar`. | CC | CONFIRMED |
| 2026-06-28 | **Arabic locale — Stage E prod re-measure + Arabic brand names.** Owner re-ran Lighthouse against the **production static export** served at `http://127.0.0.1:4123/ar/`: Desktop **95 / 100 / 96 / 100** (up from 94 on dev — confirms the page is healthy); Mobile **71 / 100 / 96 / 100**, LCP 9.6s. The remaining mobile gap is a **local-serve artifact**: `python3 -m http.server` ships assets **uncompressed and uncached**, and Lighthouse mobile throttles to slow-4G + 4× CPU, so the uncompressed JS/font payload balloons LCP. Production CloudFront serves brotli + cache headers — which is why the live `/en` `/de` score 99–100 mobile on the identical stack; the real `/ar` mobile number comes from the deployed CDN. Owner also hand-translated 7 project **brand app-names** into Arabic script for the `ar` locale (Sedra Life→سيدرا, Albruaz→البرواز, Royake→رؤياك, Orood→عروض, QRattel→رتل الحلقة الذكية, Drs Space→دكتور سبيس; plus the matching سيدرا reference inside a Sedra Life highlight). `tsc --noEmit` clean; integrity guard still satisfied (non-empty ar strings). Committed to PR #27. Serve process on :4123 already exited. | CC | CONFIRMED |
| 2026-06-28 | **Contact anchor scroll-offset fix + neutral form placeholders (EN/DE).** (1) **Anchor:** "Get in touch" (`#contact`) was landing in the empty top of the contact section. Root cause — `#contact` sat on the `<section>` box, but the section has `padding-top: var(--space-10)` = **128px** *inside* the target, above the first content (eyebrow), so the scroll parked the box top at the nav and rendered the 128px padding as an empty band. Fix is **contact-scoped** (global `NAV_OFFSET = 76` in `scrollToHash.ts` left untouched so work/about/stack land identically): moved `id="contact"` off the `<section>` onto a **non-animated zero-height anchor `<span>`** placed as the first child *inside* the padding box (its `offsetTop = sectionTop + 128`), so both motion paths land on the content. The span is non-animated on purpose — `<Reveal>`'s entrance `translateY` would shift a measured target mid-tween. Section kept as the landmark with `aria-labelledby="contact-heading"` + matching `id` on the `<h2>` (a11y accessible name). **Mobile decision (owner deferred → CC chose Option A: heading-first, never bury the heading):** at 375px the section stacks eyebrow→heading→intro→3 links→form (taller than one screen), so no offset shows heading *and* form together — landing targets the heading (clean arrival) with the form one short scroll below. Verified (dev preview, scroll mechanics are dev/prod-identical): desktop heading below nav + form in viewport, no empty band; mobile heading visible below nav + form at the fold (pre-fix it was ~128px lower, off-screen); subpage `/{locale}#contact` (via `scrollToHashOnMount`) lands identically; reduced-motion path verified by construction (same span target + shipped `scroll-padding-top: 76px` → native `scrollIntoView`); `#work`/`#about`/`#stack` unchanged; no console errors. (2) **Placeholders:** replaced misleading recruiter sample names in the `contact` namespace — EN `namePh` "Jane Recruiter"→"Your name", `emailPh` "jane@company.com"→"you@example.com"; DE `namePh` "Jana Personalerin"→"Ihr Name", `emailPh` "jana@firma.de"→"name@beispiel.de" (`msgPh` already neutral, kept; `ar.json` untouched — handled on the AR branch). `next build` + `eslint` clean, static export intact. Branch `fix/contact-anchor-and-placeholders` → PR. | CC | CONFIRMED |
| 2026-06-28 | **Favicon default override + contact anchor → form (dynamic nav offset) + neutral placeholders (en/de/ar).** (1) **Favicon root cause CONFIRMED + fixed:** the site served the Next.js logo because `src/app/favicon.ico` was the **25KB create-next-app default** — App Router auto-serves it at `/favicon.ico` and injects a hashed `<link rel="icon" type="image/x-icon">` that wins over `metadata.icons`. The branded set already existed in `public/` (favicon.svg K-dot, branded `favicon.ico` 2KB, favicon-16/32/48.png, apple-touch-icon.png, icon-192/512.png, site.webmanifest) and `layout.tsx` `metadata.icons` already declared it. **Fix = delete `src/app/favicon.ico`** (no image tooling needed; owner-approved over a full src/app migration). Verified in `out/`: `out/favicon.ico` is now the branded 2KB icon (not 25KB), head links are the clean metadata set (svg-first, no hashed `x-icon` link). Single source of truth = `public/` + `metadata.icons`. Deploy invalidates `/*`, covering all icon paths. **Caveat:** browsers cache favicons hard and Google refreshes on its own schedule — verify via hard-refresh/incognito, not Google's cached result. (2) **Contact anchor now lands on the FORM (owner revised: form fields in view, mobile+desktop):** rewrote `scrollToHash.ts` — landing target = `absoluteTop(el) − navOffset()`. `navOffset()` reads the **live `<header>` height** (the mobile nav wraps to ~172px; a hardcoded 76 couldn't clear it) + 16px gap. `absoluteTop()` sums `offsetTop` up the offsetParent chain (**transform-agnostic**, so the form's `<Reveal>` entrance translateY can't corrupt the target). Hand Lenis a **number** target (not the element) — that skips Lenis's element branch which also subtracts CSS `scroll-padding-top`, a latent **−152 double-count** (76 offset + 76 scroll-padding); native/reduced-motion path uses the same computed `top` via `window.scrollTo`. `id="contact"` moved off the section onto the `<form>`. Applied **site-wide** (owner-approved): every anchor now lands `navHeight+16` below the real nav. Verified (dev preview): mobile en/de form lands at exactly `navH(172)+16 = 188`, name input visible; desktop form **and** heading both at `nav(68)+16 = 84` (grid row); `#work`/`#about` land uniformly at +16 (the −152 double-count gone, no overshoot); ar uses the identical locale-agnostic path (its target computes correctly — only the dev emulator's stuck oversized window height clamped the scroll in-tool; a real 812px viewport scrolls fully); no console errors. (3) **Placeholders — all three catalogs:** ar `namePh` "سارة، مسؤولة التوظيف"→"اسمك", `emailPh` "jane@company.com"→"you@example.com", `msgPh`→"رسالتك"; plus `msgPh` shortened to neutral in en ("Your message") + de ("Ihre Nachricht"). `npm run lint` + `tsc --noEmit` + `next build` clean, integrity guard passes, all routes emit incl `/ar`. Branch `fix/favicon-default-and-contact-anchor` → PR. | CC | CONFIRMED |
| 2026-06-28 | **Anchor target → section orange eyebrow (all sections) + OS-based session-scoped theme.** (1) **Anchors:** every in-page anchor now lands on the section's **orange eyebrow** (the `— {eyebrow}` accent label), cleanly below the nav, uniform across Work/About/Stack/Contact. `scrollToHash.ts` was already correct (`getElementById` + `absoluteTop` offsetTop-chain − `navOffset()`); the only change is **moving the scroll-anchor `id` onto each eyebrow `<span>`** — off the `<section>` for work/about/stack, off the `<form>` for contact (the previous form-target is superseded). Contact a11y kept (`aria-labelledby="contact-heading"` + `<h2 id="contact-heading">`). Hrefs unchanged. Verified (dev preview): en desktop all four eyebrows land at `nav(68)+16 = 84`; en mobile at `nav(172)+16 = 188` (contact eyebrow at top, form below the fold = intended header-first); ar eyebrow resolves correctly (RTL, accent confirmed; the ar landing gap is the known dev-emulator oversized-window clamp, not a code bug — locale-agnostic math). (2) **Theme = OS-default + live-listen + session override:** rewrote the pre-paint `themeInit` (layout.tsx) to set `data-theme` from `sessionStorage('kp-theme')` override **else** `matchMedia('(prefers-color-scheme: dark)')` — applied synchronously in `<head>` before first paint (no FOUC); also `localStorage.removeItem('kp-theme')` to migrate off the legacy key (lang/dir block untouched). `ThemeProvider.tsx`: manual toggle writes **sessionStorage** (not localStorage); a lazy `matchMedia` listener (useReducedMotion pattern) **live-follows OS changes when there's no override**; override wins until tab close; single `data-theme` mutation point kept; `useSyncExternalStore` wiring kept. `Toggles.tsx` comment updated. **Verified full matrix (preview colorScheme emulation + sessionStorage evals):** fresh load OS light→light / dark→dark (no FOUC); live OS flip with no override follows; manual toggle → sessionStorage override; override wins over OS + persists across reload; cleared session → back to OS default; no `localStorage` key; no console errors. `lint`+`tsc`+`build` clean, all routes emit incl `/ar`. Branch `fix/anchor-eyebrow-and-os-theme` → PR. | CC | CONFIRMED |
| 2026-06-28 | **Mobile nav collapsed to an animated hamburger menu (<900px).** The mobile header was wrapping the wordmark + EN/DE/AR & LIGHT/DARK toggles + CTA across rows (~172px tall, dead whitespace). Now below the existing **900px** breakpoint the header is a **compact one row** (wordmark + hamburger, ~**69px**), with the section links + locale switcher + theme toggle + Get-in-touch CTA moved into a dropdown menu. **New `MobileMenu.tsx`** (`"use client"`): **Radix Dialog** (`@radix-ui/react-dialog`, was installed-unused, in the §2 stack) provides focus-trap, focus-return, Escape, outside-click, `aria-modal`, scroll-lock for free; **Framer `AnimatePresence`** animates open/close — the **panel stays opaque and only slides** (`translateY`, so page never bleeds through) while a **scrim fades**, and the **hamburger morphs to an X** (two bars rotate/translate) — all transform/opacity only (60fps). **Reduced-motion gate (HARD):** `useMotionActive()` (extracted to `src/lib/motion/useMotionActive.ts`, also now imported by Reveal) → when reduced, all Framer `duration: 0` so the menu opens/closes **instantly** but still works. **a11y:** real `<button>` with `aria-label`/`aria-haspopup`/`aria-expanded`/`aria-controls`; panel `role=dialog` + `aria-modal=true` + `aria-labelledby` (visually-hidden `Dialog.Title` "Menu"); focus moves into the panel on open, returns to the button on close (`onCloseAutoFocus`); `onInteractOutside` excludes the toggle button to avoid an open/close race. Menu link → `handleHashClick` (closes + scrolls to the section's orange eyebrow); **Lenis `stop()/start()`** on open/close. **RTL** via logical props (`marginInlineStart:auto`, `inset-inline`, `border-block-*`) — hamburger on the inline-end, panel + links mirrored. **Nav.tsx:** desktop toggle/CTA cluster got `.kp-nav-desktop`; renders `<MobileMenu/>`. **globals.css:** extend the 900px block — hide `.kp-nav-desktop`, show `.kp-nav-mobile` (default hidden). **Mobile header height dropped 172→~69px; eyebrow anchors still land** (`navOffset()` reads the live, now-shorter header). Verified (dev preview, en/de/ar): compact one-row header, animated panel with links+locale+theme+CTA, link→close+eyebrow scroll, RTL mirrored, Escape + X-button + scrim close, focus into panel, `aria-*` correct, no console warnings/errors; **desktop ≥900px unchanged** (links + cluster show, hamburger hidden). `lint`+`tsc`+`build` clean, all routes emit incl `/ar`. Branch `feat/mobile-hamburger-nav` → PR. | CC | CONFIRMED |
| 2026-06-28 | **Showcase `README.md` added at repo root** (recruiter / engineering-leader audience — the repo itself is part of the portfolio's proof). Sections: title + positioning + **live URL** + "the repository is part of the proof"; Highlights; **Architecture (Mermaid diagram)**; Tech stack (Web / Infra / CI-CD, real versions); Repository structure tree; Infrastructure & CI/CD prose; Notable engineering decisions; Local development / deploy; About + MIT. **Every technical claim accuracy-checked against the real repo** (read `infra/*.tf`, `lambda/contact/index.mjs`, `.github/workflows/deploy.yml`, `app/` via three Explore agents): OIDC keyless deploys (trust scoped to `repo:.../ref:refs/heads/main`), least-privilege deploy role (S3 put/delete + CF invalidate only, no GetObject), private S3 + **OAC sigv4**, HSTS + security-headers policy (**CSP omitted by design** — honest note), **directory-index CloudFront Function** (`cloudfront-js-2.0`), **two-phase ACM** (`enable_custom_domain`), API Gateway HTTP API → **Node 22 honeypot Lambda** (`hp_token`, CR/LF-stripped validation, strict-allowlist CORS never `*`) → SES, two-pass S3 cache (`_next/static` immutable / rest must-revalidate), SHA-pinned actions, Terraform remote state w/ locking, static-export i18n **en/de/ar (no middleware, `generateStaticParams`)** incl. Arabic RTL, motion gated by `prefers-reduced-motion`, **Lighthouse 100/100/100/100 desktop · 99/96/100/100 mobile**. Guardrails honored: **no Kubernetes/Docker/Helm/Argo/Prometheus** (grep-confirmed absent), no invented metrics/dollar figures, no leaked ARNs/account-ids/bucket-names/secrets (Variable/Secret **names** only). Mermaid uses conservative GitHub-safe syntax (verified). Does NOT change the deployed site. Branch `docs/readme` → PR. | CC | CONFIRMED |
| 2026-07-07 | **Server-side, privacy-scoped web analytics (Terraform, `/infra`).** Added **CloudFront standard access logs (legacy) → new private S3 log bucket → Amazon Athena** — schema-on-read over S3, **not a datastore** (§2 "no database" holds). **AGGREGATE ONLY** (page views, top pages, referrers, edge-PoP geography, traffic over time); **no client-side tracking, no cookies, no third-party, no `/app` change, no cookie banner** — zero bytes added to the browser, no Lighthouse/perf impact. **New `analytics.tf`:** log bucket `khaled-portfolio-logs-prod` (PAB all-on, AES256, **`BucketOwnerPreferred` + `awslogsdelivery` ACL grant** — legacy standard logging requires ACLs; the grant is a specific canonical user, not public, so it's PAB-compatible; TLS-deny bucket policy; no versioning; `force_destroy`); Glue db `khaled_portfolio_analytics` + external table `cloudfront_standard_logs` (33-col CloudFront format, LazySimpleSerDe tab-delim, `skip.header.line.count=2`, non-partitioned over `cf/`); Athena workgroup `khaled-portfolio-analytics` (enforced config, SSE-S3 results to `athena-results/` prefix, 1 GiB scan cap). **`cloudfront.tf`:** `logging_config` (`include_cookies=false`) attached to `aws_cloudfront_distribution.site` — **in-place update, 0 destroy**; `depends_on` the ACL. **GDPR:** viewer IPs = personal data → **S3 lifecycle auto-expire** (`log_retention_days`=90 [validation 30–90]; Athena results 14d) — CloudFront never deletes logs itself. **6 example queries** as `.sql` + provisioned `aws_athena_named_query` (page-views/day, top pages, unique-ish viewers [distinct IP + GDPR caveat], requests by **edge PoP** [proxy — standard logs have NO viewer-country; noted honestly], top referrers, requests-over-time + cache-hit ratio). New vars (`log_bucket_name`, `cf_log_prefix`, `log_retention_days`, `athena_results_retention_days`, `analytics_database_name`, `athena_workgroup_name`) + outputs; `infra/ANALYTICS.md` operator doc. **No IAM added** — queries run under the operator's own creds; the **deploy/OIDC role is NOT granted log access** (least privilege intact). **Additive only:** site bucket / OAC / ACM / CF Function / security headers / API GW / Lambda / SES / OIDC **untouched**. `terraform init -backend=false` + `validate` + `fmt -check` clean; distribution change is in-place (0 destroy) — **human runs `terraform apply`** (TF not in CI; merge to main only rebuilds the unchanged static site). Branch `feat/infra-analytics` → PR. | CC | CONFIRMED |
| 2026-07-07 | **Fixed the Top-referrers Athena saved query** (`infra/athena/queries/top-referrers.sql`). Behavior fix: the old query used `cs_referrer <> '-'` which **dropped direct traffic**; now blank/`'-'` referrers are **coalesced to `'(direct/none)'` and retained**, while same-site internal-nav referrers stay excluded (`cs_referrer NOT LIKE '%khaledhossameldin.com%'`); `GROUP BY 1`, `ORDER BY hits DESC`, fully table-qualified. **Audited all 6 saved queries against the 33-col CloudFront schema — every one already references valid columns; NO other column fixes needed** (the flagged `referrer` was only an output alias, not a column reference; underlying col is `cs_referrer`). Named queries auto-sync (`query = file(...)`), so **no `analytics.tf` change** and no other `.sql` touched. `terraform init -backend=false` + `validate` + `fmt -check` clean. Apply **replaces only `aws_athena_named_query.examples["top-referrers"]`** — Athena named queries are immutable in the API, so Terraform destroy+recreates that single saved query (lossless); **0 change/destroy on every other resource** (5 other queries, log bucket, Glue db/table, workgroup, CloudFront distribution). No `/app` change. Owner runs `terraform apply`. Branch `fix/analytics-top-referrers` → PR. | CC | CONFIRMED |
| 2026-07-07 | **Actionable analytics — CloudWatch dashboard + contact metric + Athena breakdowns** (extends the 2026-07-07 CloudFront-logs→Athena decision; aggregate-only, no PII, no client-side tracking, no cookies, no `/app` change). **(A) Real contact count:** the contact Lambda (`lambda/contact/index.mjs`) now emits ONE PII-free line `ANALYTICS contact_sent` on a **successful send only** (placed after the SES try/catch, before the final return — honeypot [L50], validation [L58–60], send-fail [L78] all return earlier and never log it; no name/email/message). New `aws_cloudwatch_log_metric_filter` on the existing Lambda log group → custom metric **`KhaledPortfolio/ContactSubmissions`** (value 1, default 0). **(B) Dashboard** `aws_cloudwatch_dashboard` **`khaled-portfolio-analytics`** (`var.dashboard_name`), 3 per-day widgets: ContactSubmissions, CloudFront `Requests` (global metric → widget pinned `region=us-east-1`), Lambda Invocations+Errors. **(C) 4 new Athena saved queries** (auto-provisioned by the existing `for_each`+`file()` — just appended names to `local.analytics_queries`): `cv-opens-by-type` (Mobile vs DevOps CV via the two exact PDF paths, `COUNT(DISTINCT c_ip)` to de-dup range requests), `project-views` (`/work/<slug>/` ranked across locales), `locale-split` (`/en`·`/de`·`/ar`), `page-views-vs-cv-opens` (per-day funnel; contacts noted as the cross-surface CloudWatch metric). **(D)** `infra/ANALYTICS.md` documents the two surfaces (CloudWatch vs Athena) + honeypot-safe metric. New file `infra/dashboard.tf`; `var.dashboard_name` + 3 outputs. **Additive: 6 to add** (metric filter, dashboard, 4 named queries) **+ 1 in-place change** (`aws_lambda_function.contact` — new zip hash redeploys code) **+ 0 destroy**; serving path (S3/OAC/ACM/CloudFront/API-GW/OIDC) untouched. `init -backend=false` + `validate` + `fmt -check` clean. Owner runs `terraform apply` (ships the Lambda line + infra), then submits a test contact → ContactSubmissions ticks up on the dashboard; runs `cv-opens-by-type` → rows. Branch `feat/infra-analytics-actionable` → PR. | CC | CONFIRMED |
| 2026-07-11 | **CV discoverability — both CVs surfaced in the hero + contact + mobile menu (were footer-only, analytics showed ≈0 opens).** The two audience-split CVs (`/Khaled_Hossameldin_Mobile_CV.pdf`, `/Khaled_Hossameldin_DevOps_CV.pdf`) were only linked at the bottom of the footer, so a recruiter never saw them. Added them to **(1) the hero** — a quiet two-link row directly under the CTAs, above the fold; **(2) the contact** direct-links list (alongside email/GitHub/LinkedIn, matching the `directLink` style); **(3) the mobile menu** (Radix Dialog panel, closes on tap). **Footer links retained.** **Pattern chosen: two plain labelled links, NOT a Radix disclosure/menu** — the hard requirement is both PDFs as **real `<a target=_blank rel=noopener>` in the static DOM** (crawlable + JS-off); a Radix menu mounts its content only on open (JS-only) → fails that, and two items don't warrant an interaction. **Consequence: no new motion anywhere → the prefers-reduced-motion gate is satisfied trivially.** New shared **`CvLinks.tsx`** (single href source; reads a new **`cv`** i18n namespace) + new **`Download`** icon in `ui/icons.tsx` (vertically symmetric → no RTL mirror) + a **`.kp-cv-link`** rule in globals (quiet→accent hover, **colour-only**, via `--cv-color`). **CV labels reused verbatim from `footer.links` across en/de/ar** (+ a new `cv.label` "CV"/"Lebenslauf"/"السيرة الذاتية" for the menu heading) — no new DE/AR wording invented. **Nav desktop bar: SKIPPED** (already carries wordmark + 4 section links + EN/DE/AR + theme toggle + CTA and wraps near the 900px breakpoint — a CV affordance would crowd it); the mobile menu covers small screens instead. **Hero top air trimmed `space-11`→`space-6` + the lead→CTA gap `space-7`→`space-6`** so the CTA **and** both CV links clear the fold in all three locales (the taller German/Arabic headlines needed the lift). Verified: `next build` + `eslint` + `tsc --noEmit` clean, static export intact (21 routes incl `/ar` + 5 detail pages); both PDF hrefs present as real `<a>` in built `out/{en,de,ar}/index.html` hero+contact (JS-off/crawlable); both PDFs return **200 `application/pdf`**; CTA+CV both above the fold at desktop 1440 (en 771 / de 885 / ar 855 px) **and** mobile 375; RTL `/ar` mirrors (content right-aligned, CV order right-to-left, download icon un-mirrored); light+dark correct; mobile hamburger shows the CV block and closes on tap. Lighthouse ≥90 expected to hold (3 anchors + 1 inline SVG, no new JS/deps, no CLS). Branch `feat/cv-discoverability` → PR → merge → OIDC deploy → live-verify on www.khaledhossameldin.com. | CC | CONFIRMED |
| 2026-07-11 | **Analytics correctness pass (`/infra`) — cv-opens root cause + bot/asset filtering + dashboard de-misleading.** The live analytics misled (the raw CloudFront `Requests` widget was read as "300–500 visitors/day"; 286 of 1484 hits were bot/scanner/tool) and `cv-opens-by-type` returned **0** despite 4 CV-PDF fetches in the logs. **cv-opens ROOT CAUSE:** the saved query required `cs_method='GET'` and matched the two PDF paths with an exact, case-sensitive `=`. A PDF "open" is logged as HTTP range/`206` + `HEAD` rows, not a clean `GET 200` (`206 < 400` already passed — so `sc_status` was fine; the **`cs_method='GET'` predicate dropped every CV row**, and the exact `=` was brittle). Rewrote it: match both PDFs via `regexp_like(lower(url_decode(cs_uri_stem)), '/khaled_hossameldin_(mobile|devops)_cv\.pdf$')`, keep `sc_status < 400` (retains 206), **drop the `GET` restriction** (doesn't make a PDF-open metric meaningful), exclude crawler UAs, de-dup with `COUNT(DISTINCT c_ip)` (NOT by filtering to non-range rows — that would re-zero it when every open is a range request) while still showing `raw_requests`. **Reusable "humans-only" filter** added across page-views-per-day, top-pages, unique-viewers-per-day, locale-split, project-views, cv-opens, and the page-views-vs-cv-opens funnel: assets (`/_next/*` + static exts incl. newly-added `.webmanifest`) + UA `bot|crawl|spider|scan|curl|wget|python|headless` on `lower(url_decode(cs_user_agent))` (CloudFront URL-encodes the UA); page metrics keep `cs_method='GET'`, CV opens deliberately don't. **New saved query `traffic-quality`** — hits + distinct IPs split **bot/scanner/tool/browser-ish** (registered in `local.analytics_queries`; honest floor caveat: UA-spoofers read as browser-ish). **Dashboard** (`dashboard.tf`, in-place): relabelled the CloudFront tile → "CloudFront requests — ALL hits incl. assets + bots (NOT visitors; see Athena)" + added a markdown header (requests ≠ page views ≠ visitors; where each real number lives; contacts = the only conversion metric). **top-referrers** already fixed in the repo (PR #36) — no file change; this apply replaces that deployed named query only if it drifted, else 0. **ANALYTICS.md** rewritten ("which number to trust" table, requests-are-NOT-visitors warning, shared-filter doc, floor caveat). **Plan shape:** 1 in-place (dashboard) + 1 add (`traffic-quality` named query) + lossless replacements of the edited named queries (Athena named queries are immutable → destroy+create of the saved SQL only) + **0 destroy** on log bucket / Glue db+table / workgroup / CloudFront distribution / S3-OAC-ACM-API GW-Lambda-OIDC. No `/app` change. `terraform init -backend=false` + `validate` + `fmt -check` clean. Owner runs `terraform apply`, then re-runs `cv-opens-by-type` (now returns the 4 known PDF hits, de-duped by IP) + `traffic-quality`. Branch `fix/infra-analytics-correctness` → PR. | CC | CONFIRMED |
| 2026-07-11 | **Repo reconcile + new DevOps CV shipped + analytics `terraform apply` completed.** Audit found everything already merged (#36 top-referrers, #37 actionable analytics, #38 CV-PDFs contact@, #39 CV discoverability, #40 analytics correctness); no open PRs, no v2 branch (redesign discarded earlier). **PART 1 — new DevOps CV:** the owner placed an updated `app/public/Khaled_Hossameldin_DevOps_CV.pdf` (adds the earned **AWS Certified Cloud Practitioner CLF-C02**; 70354 B vs old 69504 B). Committed as-is (PDF not edited) → branch `chore/update-devops-cv-aws-cert` → **PR #41 → merged → OIDC deploy green (53s)**. Mobile CV left untouched — byte-identical to the PR #38 `contact@` version (verified by git lineage + hash; no local PDF text tool was available — pdftotext/Quartz/Spotlight all absent, CID-font glyphs — so the raw email string wasn't read, but the file is provably the post-#38 one). **PART 3 — terraform:** `plan` against real state (S3 backend) showed **1 to add / 0 change / 0 destroy** — the `requests-over-time` saved query was the one named query missing from AWS (not created in a prior partial apply). Applied the **reviewed saved plan** (`terraform apply infra.tfplan`, not `-auto-approve`) → created it; **0 destroy** confirmed via `show -json` (48 no-op + 1 create, DELETES: NONE). The rest of the analytics (dashboard relabel + markdown explainer, `traffic-quality`, fixed `cv-opens-by-type`) were already applied by the owner earlier — confirmed directly against AWS. Athena now has all **11** named queries. **PART 4 — live verify (www.khaledhossameldin.com):** both CVs **200 `application/pdf`**; **DevOps CV served live sha256 == the committed cert file** (`bcc64ed1…`, 70354 B; ≠ old `5bd8bbcd…`/69504) — the "verify by fetching, not assuming" check; both CV hrefs present **×3 (hero+contact+footer) in en/de/ar**; root + `/de` + `/ar` + a `/work/<slug>/` detail route all 200; `/en` (LTR) + `/ar` (RTL) render correctly (dark), **console clean**, no regressions (asset-only change). **Analytics live:** `cv-opens-by-type` now returns the 4 known hits (**DevOps 2/2, Mobile 2/2** — no longer 0), `traffic-quality` returns the split (browser-ish 1333/199, scanner 202/10, bot 109/51, tool 29/7). Nothing left pending. Branch `chore/master-reconcile` → PR. | CC | CONFIRMED |
...
- [x] ~~**Mobile nav height (discovered 2026-06-28, contact-anchor task).** At ~375px the sticky `<header>` *wraps* to ~172px but the scroll offset was hardcoded 76px, so anchors tucked under the wrapped mobile nav.~~ RESOLVED 2026-06-28 (§10) — `scrollToHash.ts` now reads the **live `<header>` height** at scroll time (`navOffset()` = header height + 16px gap) and lands every anchor `navHeight+16` below the real nav on all viewports; also removed the latent −152 double-count by handing Lenis a number target. (Separately, a collapsed/hamburger mobile nav remains an optional design polish, not required now that the offset tracks the real height.)
...
- [x] **Analytics — needs `terraform apply` (added 2026-07-07, §10).** The CloudFront-logs → S3 → Athena stack is authored + `validate`/`fmt` clean, but is inert until a human runs `terraform apply` (AWS creds; TF not in CI). Plan should show adds + **1 in-place change** (distribution gains `logging_config`) + **0 destroy** — confirm before applying. After apply, first logs are best-effort (up to ~1h, traffic-hours only); then run a `khaled-portfolio-analytics` workgroup query to confirm data lands. Optional later: lower `log_retention_days` (30–90) for stricter data-minimization; true viewer-country (vs the edge-PoP proxy) would need real-time logs or a `CloudFront-Viewer-Country` header (out of scope for now). **Applied** (owner ran queries against live logs). Follow-up 2026-07-07 (§10): Top-referrers query patched — pends a small `terraform apply` that **replaces only the `top-referrers` named query** (0 change/destroy elsewhere). Follow-up 2026-07-07 (§10): **actionable analytics** added — CloudWatch dashboard `khaled-portfolio-analytics` + contact-submission metric (Lambda log line + metric filter) + 4 new Athena saved queries — pends `terraform apply` (6 add + 1 in-place Lambda code update, 0 destroy). After apply, submit a **test contact** to confirm `KhaledPortfolio/ContactSubmissions` increments on the dashboard. **RESOLVED 2026-07-11 (§10)** — all analytics `terraform apply` complete: dashboard (relabelled "NOT visitors" + markdown explainer), contact metric/filter, and all **11** Athena saved queries (incl. `traffic-quality`, the fixed `cv-opens-by-type`, and the last-missing `requests-over-time` created in the 2026-07-11 apply) are live; **0 destroy** on every serving-path resource. Verified live: `cv-opens-by-type` returns rows (DevOps 2/2, Mobile 2/2), `traffic-quality` returns the bot/scanner/tool/browser-ish split.
