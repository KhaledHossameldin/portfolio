...
- [ ] Contact honeypot field renamed `company` → `hp_token` (2026-06-18, §10) in both Contact.tsx and the Terraform-deployed Lambda. The Lambda ships via `terraform apply`, NOT deploy.yml — run an apply so the live function checks `data.hp_token` before the contact endpoint is wired, or the server honeypot reverts to the old (inert) `company` field. No current prod drift (endpoint not yet live).
