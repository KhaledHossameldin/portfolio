...
- [ ] **Mobile nav height (discovered 2026-06-28, contact-anchor task).** At ~375px the sticky `<header>` *wraps* (`flexWrap: wrap`: wordmark + lang/theme toggles + CTA) to **~172px** tall, but the global scroll offset (`NAV_OFFSET`/`scroll-padding-top`) is **76px** — so on mobile a section's small eyebrow can tuck just under the wrapped nav (the heading still clears it). Affects **all** sections, pre-existing, not introduced by the anchor fix. Options: a collapsed/hamburger mobile nav, or make the scroll offset = live `header` height. Out of scope for the anchor fix (would touch global motion + Nav); revisit as a mobile-nav polish.
